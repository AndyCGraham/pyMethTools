...
class pyMethObj():
    ...
    def fit_betabinom(self,chunksize: int=1,ncpu: int=1,start_params=[]):
        """
        ...
        if ncpu > 1: #Use ray for parallel processing
            ...
            if chunksize>1:
                ...
                self.fits = ray.get([self.fit_region.remote(cpg_id,region,meth_id,coverage_id,region_id,X_id,X_star_id,self.maxiter,self.maxfev,start_params,self.param_names_abd,self.param_names_disp) 
    ...
    @staticmethod
    @ray.remote
    def fit_cpg(cpg,meth_id,coverage_id,X_id,X_star_id,maxiter=150,maxfev=150,start_params=[],param_names_abd=["intercept"],param_names_disp=["intercept"]):
        """
        Perform differential methylation analysis on a single cpg using beta binomial regression.
    ...
    @staticmethod
    def corncob_region_local(region,fits,min_cpgs=3,param_names_abd=["intercept"],param_names_disp=["intercept"]):
        """
        ...
        X_c = np.concatenate([fits[start].X,fits[start].X,fits[start].X]) 
        X_star_c = np.concatenate([fits[start].X_star,fits[start].X_star,fits[start].X_star]) 
        ...
        while end < len(fits)+1:
            ...
            if start+min_cpgs < len(fits)+1:
                ll_c = corncob_cpg_local("",np.hstack([fit.count for fit in fits[start:max(start+min_cpgs,end)]]),
                                             np.hstack([fit.total for fit in fits[start:max(start+min_cpgs,end)]]),X_c,X_star_c,region=region,LL=True,res=False,
                                         param_names_abd=param_names_abd,param_names_disp=param_names_disp)
                ...
                if bic_c < bic_s:
                    ...
                    if (end-start) > min_cpgs+1: #Save region if number of cpgs > min_cpgs
                        ...
                        region_res+=[corncob_cpg_local(f'{start}-{end-3}',np.vstack([fit.count for fit in fits[start:end-2]]).sum(axis=0),np.vstack([fit.total for fit in fits[start:end-2]]).sum(axis=0),
                                                       fits[start].X,fits[start].X_star,region=region,param_names_abd=param_names_abd,param_names_disp=param_names_disp)] #Add regions results to results table
                    start=end-2
                    X_c = np.concatenate([fits[start].X,fits[start].X,fits[start].X]) 
                    X_star_c = np.concatenate([fits[start].X_star,fits[start].X_star,fits[start].X_star]) 
        ...
        if (end-start) > min_cpgs+1: #Save final region if number of cpgs > min_cpgs
            region_res+=[corncob_cpg_local(f'{start}-{end}',np.vstack([fit.count for fit in fits[start:end]]).sum(axis=0),np.vstack([fit.total for fit in fits[start:end]]).sum(axis=0),
                                           fits[start].X,fits[start].X_star,region=region,param_names_abd=param_names_abd,param_names_disp=param_names_disp)]
    ...
    @staticmethod
    @ray.remote
    def corncob_region(region,region_id,fits_id,min_cpgs=3,param_names_abd=["intercept"],param_names_disp=["intercept"]):
        """
        Perform differential methylation analysis on a single target region using beta binomial regression, with an option to compute differentially methylated regions of contigous cpgs whose
        mean and association with covariates are similar.
        ...
        while end < len(fits)+1:
            ...
            if start+min_cpgs < len(fits)+1:
                ll_c = corncob_cpg_local("",np.hstack([fit.count for fit in fits[start:max(start+min_cpgs,end)]]),
                                             np.hstack([fit.total for fit in fits[start:max(start+min_cpgs,end)]]),X_c,X_star_c,maxiter,maxfev,region=region,LL=True,res=False,
                                         param_names_abd=param_names_abd,param_names_disp=param_names_disp)
                ll_s = np.array([fits[cpg].LogLike for cpg in range(start,max(start+min_cpgs,end))]).sum()
                ...
                if bic_c < bic_s:
                    ...
                    if (end-start) > min_cpgs+1: #Save region if number of cpgs > min_cpgs
                        current_region+=1
                        region_res+=[corncob_cpg_local(f'{start}-{end-3}',np.vstack([fit.count for fit in fits[start:end-2]]).sum(axis=0),np.vstack([fit.total for fit in fits[start:end-2]]).sum(axis=0),
                                                       fits[start].X,fits[start].X_star,maxiter,maxfev,region=region,param_names_abd=param_names_abd,param_names_disp=param_names_disp)] #Add regions results to results table
                    start=end-2
                    X_c = np.concatenate([fits[start].X,fits[start].X,fits[start].X]) 
                    X_star_c = np.concatenate([fits[start].X_star,fits[start].X_star,fits[start].X_star]) 
        ...
        if (end-start) > min_cpgs+1: #Save final region if number of cpgs > min_cpgs
            region_res+=[corncob_cpg_local(f'{start}-{end}',np.vstack([fit.count for fit in fits[start:end]]).sum(axis=0),np.vstack([fit.total for fit in fits[start:end]]).sum(axis=0),
                                           fits[start].X,fits[start].X_star,maxiter,maxfev,region=region,param_names_abd=param_names_abd,param_names_disp=param_names_disp)]
