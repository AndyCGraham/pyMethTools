...
import ray
import warnings
from pyMethTools.bbll import bbll
...
class Corncob_2():
    ...
    def __init__(self, total, count, X=None, X_star=None, phi_init=0.5,param_names_abd=["intercept"],param_names_disp=["intercept"]):
        ...
        self.df_residual = X.shape[0] - self.df_model
    ...
    def objective(self, beta):
        return bbll(beta, self.X, self.X_star, self.count, self.total, self.n_param_abd, self.n_param_disp)
...
def corncob_cpg_local(site,meth,coverage,X,X_star,maxiter=500,maxfev=500,region=None,res=True,LL=False,param_names_abd=["intercept"],param_names_disp=["intercept"]):
    """
    Perform differential methylation analysis on a single cpg using beta binomial regression.
